...
router.get('/', authenticate, async (req, res, next) => {
  try {
    ...
    const result = await query(
      `SELECT id, email, full_name, avatar_url FROM users WHERE (full_name LIKE ? OR email LIKE ?) AND id != ? LIMIT 10`,
      [`%${q}%`, `%${q}%`, req.user.id]
    );
...
router.patch('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    await query(
      `UPDATE users SET full_name = COALESCE(?, full_name), bio = COALESCE(?, bio), avatar_url = COALESCE(?, avatar_url) WHERE id = ?`,
      [fullName || null, bio !== undefined ? bio : null, avatarUrl || null, req.user.id]
    );
    const result = await query('SELECT id, email, full_name, bio, avatar_url, role FROM users WHERE id = ?', [req.user.id]);
