...
function getPasswordStrength(pwd: string): { level: string; label: string; class: string } {
  ...
  if (score <= 1) return { level: 'weak', label: 'Weak', class: 'strength-weak' };
  if (score <= 2) return { level: 'fair', label: 'Fair', class: 'strength-fair' };
  if (score <= 3) return { level: 'good', label: 'Good', class: 'strength-good' };
    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden bg-gray-950">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10 animate-fade-in">
          ...
          <h1 className="text-xl font-semibold text-white font-display">Create your account</h1>
          <p className="text-sm text-gray-400 mt-1.5">Start managing projects in minutes</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-8 animate-slide-up">
          ...
          <form onSubmit={handleSubmit} className="space-y-5" id="register-form">
            ...
            <div>
              <label htmlFor="register-email" className="block text-sm font-medium text-gray-300 mb-1.5">Email</label>
              <input
                id="register-email"
                type="email"
                value={email}
            ...
            <div>
              ...
              {password && (
                <div className="mt-2.5 space-y-1.5">
                  ...
                  <p className={`text-xs font-medium ${strengthColors[strength.level] || 'text-gray-600'}`}>
                    {strength.label}
                  </p>
                </div>
              )}
